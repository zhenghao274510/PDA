<template>
	<div class="show-box" :class="{disabled: disabled, active: isShowList}" style="style_Container" @click.stop>
		...
		<!-- 右侧的小三角图标 -->
		<span @click.stop="onUpperClick" class="iconfont iconarrowBottom-fill right-arrow" :class="{isRotate: isRotate}"></span>

		<!-- 清除按钮图标 -->
		<span v-if="clearable && selectText && selectText != placeholder" class="right-arrow" @click.stop="onClear">
			<span class="iconfont iconshanchu1 clear"></span>
		</span>


		<!-- 列表框 -->
		<div class="list-container" @click.stop="onListClick" :style="'top:' + listTop + 'px;'" v-show="isShowList">
			...
			<scroll-view class="list" style="background-color: #fff;" :style="'max-height: ' + listBoxHeight__ +'em;'" scroll-y=true
			 scroll-x=true>
				<div class="item" @click.stop="onClickItem(index, item.value)" v-for="(item, index) in innerList" :key="index"
