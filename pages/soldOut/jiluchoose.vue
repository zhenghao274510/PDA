		...
		<view class="uni-add-swiper page-top-padding">
			<view class="uni-add-item">
				...
				<view class="uni-add-list">
					...
					<xfl-select :list="storeList" :clearable="false" :listShow="false" :isCanInput="false" :placeholder="'placeholder'"
	...
	export default {
		...
		methods: {
			...
			onConfirm(e) {
				...
				switch (n) {
					case 0:
						this.putOffDateStart = e.result;
						break;
					case 1:
						this.putOffDateEnd = e.result;
						break;
					case 2:
			...
			findItem() {
				...
				if (this.putOffDateStart != '开始日期') {
					parmas.putOffDateStart = this.putOffDateStart
				}
				if (this.updaterName != '') {
					parmas.updaterName = this.updaterName
				}
				if (this.putOffDateEnd != '结束日期') {
					parmas.putOffDateEnd = this.putOffDateEnd
				}
				...
				this.$api.navTo(
					`/pages/soldOut/soldOutHistory?id=${JSON.stringify(parmas)}`
				)
			}
