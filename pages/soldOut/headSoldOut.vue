	...
	<view>
		<cover-view class="uni-page-scan-con">
			<cover-view class="uni-page-scan-left">
				...
				<cover-view class="uni-page-scan-title" style="color: #FFFFFF;">{{lang.title.Underwork}}</cover-view>
			</cover-view>
...
<script>
	...
	export default {
		...
		onLoad(d) {
			this.content = this.lang.title.Handoffshelves;
		...
		methods: {
			onClick(n) {
				n == 0 ? (uni.navigateBack(), barcode.close()) : this.$api.navTo("/pages/soldOut/soldOutHistory")
			},
			// 扫码成功回调
			...
			createView(currentWebview) {
				...
				content.addEventListener("click", () => {
					uni.navigateTo({
						url: "/pages/soldOut/newSoldOut?type=1"
					})
				});
				var temp = this;
