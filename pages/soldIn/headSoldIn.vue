	...
	<view>
		<cover-view class="uni-page-scan-con">
			<cover-view class="uni-page-scan-left">
				...
				<cover-view class="uni-page-scan-title" style="color: #FFFFFF;">{{lang.title.Onmyhomework}}</cover-view>
			</cover-view>
...
<script>
	...
	export default {
		...
		onLoad(d) {
			this.content=this.lang.title.Manualon;
		...
		methods: {
			onClick(n) {
				n == 0 ? (uni.navigateBack(), barcode.close()) : this.$api.navTo("/pages/soldIn/soldInHistory?type=1")
			},
			// 扫码成功回调
			onmarked(type, result) {
				...
				this.$api.navTo(`/pages/soldOut/newSoldOut?id=${result}&type=0`)
			...
			createView(currentWebview) {
				...
				content.addEventListener("click", () => {
					uni.navigateTo({
						url: "/pages/soldIn/newSoldIn?type=1"
					})
				});
				var temp = this;
